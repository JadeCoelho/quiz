...
function piscarNoAcerto(r) {
  r.classList.remove("errou");
  r.classList.add("acertou");
}

function piscarNoErro(r) {
  r.classList.remove("acertou");
  r.classList.add("errou");
}

function tirarPiscar(r) {
  r.classList.remove("acertou");
  r.classList.remove("errou");
}
...
function verificarSeAcertou(nQuestao, resposta) {
  ...
  if (respostaEscolhida == certa) {
    piscarNoAcerto(resposta);
    pontos += 10; // pontos = pontos + 10
    if (nQuestao.value == 1 && pontos == 20) {
      pontos = 10;
    }
  } else {
    piscarNoErro(resposta);
  }
  setTimeout(() => {
    tirarPiscar(resposta);
  }, 150);
